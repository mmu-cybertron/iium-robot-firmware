...
void app_main(void)
{
    ...
    /* FIX: Changed from while to do-while to ensure initial move function is invoked
       first, switching state flag from 0 to 1 before checking if it's done. */
    do {
        /* Call initial move executor */
        game_mode_selector_execute_initial_move();
...
#endif /* ROBOT_GAME_MODE_SELECTOR_ENABLE */
}
